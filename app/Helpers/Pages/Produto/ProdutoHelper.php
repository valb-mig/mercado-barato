...
class ProdutoHelper
{
    public static function getDadosProduto(int $id):object
    {
        $produto = Produtos::select('mb_produtos.*','s.setor_nome')
            ->join('mb_setores as s', 'mb_produtos.id_setor', 's.id')
            ->where('mb_produtos.id', $id)
            ->first();
